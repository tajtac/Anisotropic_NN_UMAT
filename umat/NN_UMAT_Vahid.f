...
c...  initialize material parameters
      ...
      !   write(7,*) "=============================="
      !   write(7,*) Allweights(4), Allweights(5), Allweights(6)
      !   write(7,*) n_input
      !   write(7,*) weight_count
      !   write(7,*) bias_count
      !   write(7,*) n_neuronsperlayer
      !   write(7,*) I1mean
      !   write(7,*) Psivar
      !   write(7,*) "s0"
      !   write(7,*) s0vector
...
C       do i=1,3
C         do j=1,3
C           C(i,j) = dfgrd1(1,i)*dfgrd1(1,j) + dfgrd1(2,i)*dfgrd1(2,j) + 
C      #             dfgrd1(3,i)*dfgrd1(3,j)
C         end do
C       end do
...
c... below the index of bias_count+1 corresponds to the first output, which is Psi
      Psi1  = output_vector(bias_count+1)**2
      Psi2  = output_vector(bias_count+2)**2
      Psi4a = output_vector(bias_count+3)**2
      Psi4s = output_vector(bias_count+4)**2
c... and second derivatives Psi11,Psi12,Psi21,Psi22
      Psi11   = output_grad((0+bias_count)*n_input+1)*output_vector(bias_count+1)*2
      Psi12   = output_grad((0+bias_count)*n_input+2)*output_vector(bias_count+1)*2
      Psi14a  = output_grad((0+bias_count)*n_input+3)*output_vector(bias_count+1)*2
      Psi14s  = output_grad((0+bias_count)*n_input+4)*output_vector(bias_count+1)*2
      Psi21   = output_grad((1+bias_count)*n_input+1)*output_vector(bias_count+2)*2
      Psi22   = output_grad((1+bias_count)*n_input+2)*output_vector(bias_count+2)*2
      Psi24a  = output_grad((1+bias_count)*n_input+3)*output_vector(bias_count+2)*2
      Psi24s  = output_grad((1+bias_count)*n_input+4)*output_vector(bias_count+2)*2
      Psi4a1  = output_grad((2+bias_count)*n_input+1)*output_vector(bias_count+3)*2
      Psi4a2  = output_grad((2+bias_count)*n_input+2)*output_vector(bias_count+3)*2
      Psi4a4a = output_grad((2+bias_count)*n_input+3)*output_vector(bias_count+3)*2
      Psi4a4s = output_grad((2+bias_count)*n_input+4)*output_vector(bias_count+3)*2
      Psi4s1  = output_grad((3+bias_count)*n_input+1)*output_vector(bias_count+4)*2
      Psi4s2  = output_grad((3+bias_count)*n_input+2)*output_vector(bias_count+4)*2
      Psi4s4a = output_grad((3+bias_count)*n_input+3)*output_vector(bias_count+4)*2
      Psi4s4s = output_grad((3+bias_count)*n_input+4)*output_vector(bias_count+4)*2
c... Revert normalization
...
c...          here I include the J^(-4/3) term inside CChat rather than multiplying it later.
...
c...  Abaqus corrections 
     ...
     #                    +kron(j,l)*sigma(i,k))
      ...
      !   write(7,*) "Chat"
      !   write(7,*) Chat
      !   write(7,*) "I1hat, I2hat, I4ahat, I4shat"
      !   write(7,*) I1hat, I2hat, I4ahat, I4shat
      !   write(7,*) "Psi1, Psi2, Psi4a, Psi4s"
      !   write(7,*) Psi1, Psi2, Psi4a, Psi4s
      !   write(7,*) "S^hat"
      !   write(7,*) Shat
      !   write(7,*) "PP1"
